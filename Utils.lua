...
-- LibSharedMedia
-------------------------------------------------
Cell.vars.texture = "Interface\\AddOns\\Cell_Ascension\\Media\\statusbar.tga"
Cell.vars.emptyTexture = "Interface\\AddOns\\Cell_Ascension\\Media\\empty.tga"
Cell.vars.whiteTexture = "Interface\\AddOns\\Cell_Ascension\\Media\\white.tga"
...
-- Cell_Ascension now operates in FETCH-ONLY mode with LibSharedMedia.
...
-- - By not registering, Cell_Ascension won't trigger any callbacks or interfere with other addons
-- - Cell's resources remain available through direct paths for Cell's internal use

-- No-op function for compatibility (called from Core_Ascension.lua)
...
function F.GetBarTextureByName(name)
    ...
    return "Interface\\AddOns\\Cell_Ascension\\Media\\statusbar.tga"
end
...
function F.GetFont(font)
    ...
    elseif type(font) == "string" and strfind(strlower(font), ".ttf$") then
        ...
        if CellDB["appearance"]["useGameFont"] then
            ...
            return "Interface\\AddOns\\Cell_Ascension\\Media\\Fonts\\Accidental_Presidency.ttf"
        end
    end
end
...
function F.GetTextures()
    ...
    for _, s in pairs(shapes) do
        tinsert(t, "Interface\\AddOns\\Cell_Ascension\\Media\\Shapes\\"..s..".tga")
    end
...
function F.GetDefaultRoleIcon(role)
    ...
    return "Interface\\AddOns\\Cell_Ascension\\Media\\Roles\\Default_" .. role
end

function F.GetDefaultRoleIconEscapeSequence(role, size)
    if not role or role == "NONE" then return "" end
    return "|TInterface\\AddOns\\Cell_Ascension\\Media\\Roles\\Default_" .. role .. ":" .. (size or 0) .. "|t"
end
...
if C_Spell and C_Spell.IsSpellInRange then
    UnitInSpellRange = function(spellName, unit)
        return IsSpellInRange(spellName, unit) == true or IsSpellInRange(spellName, unit) == 1
    end
else
    UnitInSpellRange = function(spellName, unit)
...
local function SPELLS_CHANGED()
    ...
    if spell_friend then
        ...
        spell_friend = name or nil -- skip IsSpellKnownOrOverridesKnown: Ascension custom spells may fail it
    end
...
function F.IsInRange(unit, check)
    ...
    elseif not check and F.UnitInGroup(unit) then
        ...
        -- UnitInRange may use a shorter range than actual spell range on Ascension;
