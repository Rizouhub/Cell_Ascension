...
local function CreateCodeSnippetsFrame()
    ...
    newBtn.tex:SetTexture("Interface\\AddOns\\Cell_Ascension\\Media\\Icons\\add.tga")
    ...
    -- current line number
    local lineNumber = codeSnippetsFrame.header:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_CLASS")
    ...
    errorPopup.close = Cell.CreateButton(errorPopup, "X", "red", {18, 18}, false, false, "Cell_Ascension_FONT_WIDGET_TITLE", "Cell_Ascension_FONT_WIDGET_TITLE_DISABLE")
    errorPopup.close:SetPoint("TOPRIGHT")
    errorPopup.close:SetScript("OnClick", function()
        errorPopup:Hide()
    end)

    errorPopup.text = errorPopup:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
...
LoadList = function()
    ...
    if not buttons[0] then
        ...
        -- label
        buttons[0].label = buttons[0]:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
    ...
    for i, t in ipairs(CellDB["snippets"]) do
        if not buttons[i] then
            ...
            buttons[i].del.tex:SetTexture("Interface\\AddOns\\Cell_Ascension\\Media\\Icons\\close.tga")
            ...
            -- label
            buttons[i].label = buttons[i]:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
