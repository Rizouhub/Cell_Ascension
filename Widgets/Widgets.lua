...
font_special:SetFont("Interface\\AddOns\\Cell_Ascension\\Media\\Fonts\\font.ttf", 12, "")
...
function Cell.UpdateOptionsFont(offset, useGameFont)
    if useGameFont then
        ...
        defaultFont = "Interface\\AddOns\\Cell_Ascension\\Media\\Fonts\\Accidental_Presidency.ttf"
    end
...
function Cell.CreateMovableFrame(title, name, width, height, frameStrata, frameLevel, notUserPlaced)
    ...
    header.closeBtn = Cell.CreateButton(header, "X", "red", {20, 20}, false, false, "Cell_Ascension_FONT_WIDGET_TITLE", "Cell_Ascension_FONT_WIDGET_TITLE_DISABLE")
...
function Cell.CreateTipsButton(parent, size, points, ...)
    ...
    tips.tex:SetTexture("Interface\\AddOns\\Cell_Ascension\\Media\\Icons\\info2.tga")
...
function Cell.CreateCheckButton(parent, label, onClick, ...)
    ...
    -- cb:SetDisabledCheckedTexture([[Interface\AddOns\Cell_Ascension\Media\CheckBox\CheckBox-DisabledChecked-16x16]])
...
function Cell.CreateStatusBarButton(parent, text, size, maxValue, template)
    ...
    bar:SetStatusBarTexture("Interface\\AddOns\\Cell_Ascension\\Media\\statusbar.tga")
...
local function CreateItemButtons_Scroll(items, itemTable, limit, level)
    ...
    for i, item in pairs(items) do
        ...
        --     end
        --     b.marker:SetTexture("Interface\\AddOns\\Cell_Ascension\\Media\\Icons\\icon_marker.tga")
...
function Cell.CreateDropdown(parent, width, dropdownType, isMini, isHorizontal)
    ...
    if isMini then
        ...
        menu.button:SetFrameLevel(menu:GetFrameLevel()+1)
        menu.button:SetNormalTexture([[Interface\AddOns\Cell_Ascension\Media\Icons\dropdown-normal]])
        menu.button:SetPushedTexture([[Interface\AddOns\Cell_Ascension\Media\Icons\dropdown-pushed]])

        local disabledTexture = menu.button:CreateTexture(nil, "OVERLAY")
        disabledTexture:SetTexture([[Interface\AddOns\Cell_Ascension\Media\Icons\dropdown-normal]])
...
function Cell.CreateBindingButton(parent, width)
    if not parent.bindingButton then
        ...
        parent.bindingButton.close = Cell.CreateButton(parent.bindingButton, "X", "red", {18, 18}, true, true, "Cell_Ascension_FONT_WIDGET_TITLE", "Cell_Ascension_FONT_WIDGET_TITLE_DISABLE")
...
function Cell.CreateReceivingFrame(parent)
    ...
    -- labels
    local typeLabel = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_CLASS")
    typeLabel:SetPoint("TOPLEFT", 10, -10)
    typeLabel:SetText(L["Type: "])

    local nameLabel = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_CLASS")
    nameLabel:SetPoint("LEFT", 10, 0)
    nameLabel:SetText(L["Name: "])

    local fromLabel = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_CLASS")
    fromLabel:SetPoint("LEFT", 10, 0)
    fromLabel:SetText(L["From: "])

    local dataLabel = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_CLASS")
    dataLabel:SetPoint("LEFT", 10, 0)
    dataLabel:Hide()

    -- texts
    local typeText = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
    ...
    local nameText = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
    ...
    local fromText = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
    ...
    local dataText = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
    ...
    -- error
    local infoMsg = f:CreateFontString(nil, "OVERLAY", "Cell_Ascension_FONT_WIDGET")
    ...
    -- bar
    local progressBar = Cell.CreateStatusBar("CellReceivingFrameBar", f, 198, 18, 0, true, nil, true, "Interface\\AddOns\\Cell_Ascension\\Media\\statusbar.tga", {0.7, 0.7, 0, 1})
